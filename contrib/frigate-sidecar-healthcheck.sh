#!/bin/sh
# Restart frigate-sidecar after N consecutive /healthz failures.
#
# Run by frigate-sidecar-healthcheck.timer every 60s. A single 503 is not a
...
CONFIG="${FRIGATE_SIDECAR_CONFIG:-/opt/frigate-sidecar/config/sidecar.yml}"
UNIT="${FRIGATE_SIDECAR_UNIT:-frigate-sidecar}"
THRESHOLD="${FRIGATE_SIDECAR_HEALTHCHECK_FAILS:-3}"
