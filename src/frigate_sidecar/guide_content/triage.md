...
the [score tuning](/guide/tuning-scores) workflow — a few labeling sessions
give you the evidence to set per-camera thresholds instead of guessing.

## The list

Filter by camera, object label, triage state, time window, and sort order.
The header shows running TP/FP/SKIP counts, and a **session** field lets you
tag a batch of labels (e.g. `2026-08-23a`) so you can tell labeling passes
apart later.

## The event page

Tap any event to open `/event/{id}`:
